...
ds.age.I <- function(vAge.age,show = TRUE) {
  ...
  if (show == TRUE) {
    ...
    abline(
      v = c(age.low,age.avg,age.upper),col = 6,lty = 5
    )
  }
  vAge.I <- vAge.age
...
ds.accumRate <- function(vData,start = 1,end = 1) {
  if (length(which(is.na(vData))) > 0 ||
      length(which(is.infinite(vData))) > 0) {
    stop("ERROR:there is NA value!")
    # return(c(accumRate=-1111,STD=-1111))
  }
  if (end > length(vData) || end < start ||
      start < 1 || length(vData) <= 2) {
  ...
  vSTD <-
    sqrt(var(diff(vData.tmp) / vData.tmp[1:(length(vData.tmp) - 1)]))
...
### ds.mdd ----
# get the max draw down ratio of a sequence
# this function is an expanded maxdrawdown in package "tseries"
# the default type is "ratio"; and an alternative type is "value",
# it could get the max draw down value of a sequence.
ds.mdd <- function(x, type = "ratio") {
  if (NCOL(x) > 1)
    stop("x is not a vector or univariate time series")
  if (any(is.na(x)))
    stop("NAs in x")
  if (type == "ratio") {
    cmaxx <- (cummax(x) - x) / cummax(x)
  }else if (type == "value") {
    # cmaxx <- cummax(x) - x  # the form in package "tseries"
  }else{
    stop("type is error!")
  }
  mdd <- max(cmaxx)
  ...
  return(list(
    maxdrawdown = mdd, from = from, to = to
  ))
}
...
ds.evaluate.seq <- function(vData) {
  ...
  vMdd <- ds.mdd(vData)
  vMdd.ratio <- vMdd$maxdrawdown
  if (vMdd$maxdrawdown == 0)
    vMdd.ratio <- 0
  if (NROW(vData) < 2)
  ...
  return(
    data.frame(
      accumRate = vAccumRate["accumRate"],accumRate.avg = vAccumRate.avg,
      accumRate.min = vAccumRate.min,STD = vAccumRate["STD"],STD.max =
        vSTD.max,
      mdd = vMdd.ratio
    )
  )
...
ds.dbConnect <- function(conf,encoding = 'UTF8') {
  library(RMySQL)
  con <- dbConnect(
    MySQL(), user = conf$user, password = conf$password,
...
ds.getSQL.Data <- function(conf, sqlBody,encoding = 'UTF8') {
  ...
  con <- dbConnect(
    MySQL(), user = conf$user, password = conf$password,
    dbname = conf$dbname, host = conf$host, client.flag =
...
ds.writeSQL.Data <-
  function(conf,vTable,vInsertData,apnd = TRUE,overwrte = FALSE,rownm = FALSE,encoding =
             'UTF8') {
    ### Write the result into the database
    con <- ds.dbConnect(conf,encoding = encoding)
    vSQLRes <-
      dbWriteTable(
        con,vTable,vInsertData,append = apnd,overwrite = overwrte,row.names = rownm
      )
    if (vSQLRes) {
      dbDisconnect(con)
...
ds.annReturn <-
  function(vData,scale = 252,geometric = TRUE,type = "seq",len = 1) {
    if (type == "seq") {
...
ds.workdays <- function(vDate, vHolidays = NULL) {
  ...
  vDate <-
    as.Date(setdiff(as.character(vDate), as.character(vHolidays)))
  return(vDate)
}
...
ds.optCombWght <-
  function(vCombFund.m, vThreshold.mdd, Ifshow = TRUE,tries = 2500) {
    library(PerformanceAnalytics)
...
ds.quantEvaList <- function(vData) {
  ...
  vAnnR <-
    as.numeric(ds.annReturn(vData)) # clear the names by using as.numeric
  vCalmaR <- vAnnR / vES.arry$mdd
  return(
    c(
      accumReturn = vES.arry$accumRate,annReturn = vAnnR,vSR.arry[,c("volatility","vSR")],
